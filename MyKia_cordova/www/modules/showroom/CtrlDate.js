/**
 * Showroom > Test Drive > Date Picker Controller 
 *
 * @ Develop Desc 		: Showroom Test Drive Date Picker Controller
...
define([
	...
	/**
	 * declare showroom > Test Drive > Date Picker Controller
	 */
	module.controller('AppointmentsShowroomDateController', [
									 '$scope'
									,'$rootScope'
									,'$state'
									,'$filter'
									...
									,'service.utils'
									,'ROUTE_URL'
									,AppointmentsShowroomDateCtrl
								]);

	/**
	 * inject showroom > Test Drive > Date Picker Controller into module
	 * @ $scope				: angular scope
	 * @ $log 				: angular log
	 * @ Network 			: myKiaApp.services.network Service
	 */
	function AppointmentsShowroomDateCtrl($scope, $rootScope, $state, $filter, $window, 
																	Network, Utils, ROUTE_URL) {
		
		// set navigation title
		// 상단 Navigation 설정
		$rootScope.setNaviTitle( ROUTE_URL.SHOWROOM_DATE_PICKER );
		...
		$scope.nextstep = function() {
			...
			$rootScope.showroom.date = date;
			$rootScope.showroom.time = time;
			$rootScope.showroom.ScheduledDate = $scope.selected.format('YYYY-MM-DDT') + moment(time.Start, 'YYYY-MM-DD HH:mm:SS').format('HH:mm:SS');
			$rootScope.showroom.Comment = comment;
			// $rootScope.showroom.CallbackByDealer = callbackByDealer;
			$rootScope.showroom.Campaigns = $scope.kmeCheckboxGroup.selectedCampaign;

			//console.log($rootScope.showroom)
